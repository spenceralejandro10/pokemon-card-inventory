...
const state={cards:[],favorites:new Map(),offers:{},order:null};
...
function renderFavoriteItems(){const box=document.getElementById("favoriteItems");box.innerHTML="";[...state.favorites.values()].filter(Boolean).forEach(card=>{const row=document.createElement("div");row.className="favorite-item";row.innerHTML=`<div><strong>${card.canonical_name||card.name_original||"Carta"}</strong><small>${card.card_number||card.id}</small></div><input type="number" min="0" step="1000" placeholder="Tu oferta" value="${state.offers[card.id]||""}">`;row.querySelector("input").oninput=e=>{state.offers[card.id]=Number(e.target.value||0);document.getElementById("lotOffer").value="";updateOfferTotal()};box.appendChild(row)});updateOfferTotal()}
function updateOfferTotal(){const lot=Number(document.getElementById("lotOffer").value||0),sum=Object.values(state.offers).reduce((a,b)=>a+Number(b||0),0);document.getElementById("offerTotal").textContent=cop(lot||sum)}
document.getElementById("lotOffer").oninput=updateOfferTotal;
document.getElementById("sendOffer").onclick=()=>{const lot=Number(document.getElementById("lotOffer").value||0);const lines=[...state.favorites.values()].filter(Boolean).map(c=>`• ${c.canonical_name||c.name_original||"Carta"} ${c.card_number||c.id}: ${lot?"incluida en oferta del lote":cop(state.offers[c.id]||0)}`);window.open(`https://wa.me/${WA}?text=${encodeURIComponent("Hola, quiero negociar estas cartas Pokémon:\n"+lines.join("\n")+"\n\nOferta total: "+cop(lot||Object.values(state.offers).reduce((a,b)=>a+Number(b||0),0)))}`,"_blank")};
